...
RenderingRule::RenderingRule(Attributes const & attrs, bool isGroup, RenderingRulesStorage* storage) {
...
bool RenderingRuleSearchRequest::checkInputProperties(RenderingRule const * rule) {
...
bool RenderingRuleSearchRequest::visitRule(RenderingRule const * rule, bool loadOutput) {
	...
	for (j = 0; j < rule->ifElseChildren.size(); j++) {
		match = visitRule(rule->ifElseChildren[j], loadOutput);
		if (match) {
			break;
		}
	}
